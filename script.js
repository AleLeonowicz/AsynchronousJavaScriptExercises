...
const getCountryData = function (country) {
  ...
  request.addEventListener('load', function () {
    ...
    console.log('----------------------------');
    console.log('data', data.name.official);

    const countryCurrency = Object.values(data.currencies)[0].name;
    console.log(
...
const fetchMyLocation = async (lat, lng) => {
  try {
...
const whereAmI = async function () {
  try {
    ...
    const resGeo = await fetchMyLocation(lat, lng);
    ...
    // Reject promise retrned from async function
    throw err;
  }
};
...
get3Countries('poland', 'norway', 'czech republic');
...
// Promise.allSettled
// (returns all the results of all the promises, while Promise.all returns only when all are resolved)
...
const loadAll = async function (imgArr) {
  try {
    ...
    const finalImgs = await Promise.all(imgs);
    console.log(finalImgs);

    finalImgs.forEach(finalImg => finalImg.classList.add('parallel'));
  } catch (err) {
    console.error(err);
  }
};

const arr1 = ['img/img-1.jpg', 'img/img-2.jpg', 'img/img-3.jpg'];
loadAll(arr1);
